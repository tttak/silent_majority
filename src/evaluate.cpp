...
namespace {
    EvalSum doapc(const Position& pos, const EvalIndex index[2]) {
        ...
        const EvalIndex* list0 = (EvalIndex*)pos.cplist0();
        const EvalIndex* list1 = (EvalIndex*)pos.cplist1();
    ...
    bool calcDifference(Position& pos, Search::Stack* ss) {
        ...
        if (lastMove.pieceTypeFrom() == King) {
            ...
            if (pos.turn() == Black) {
                ...
                const EvalIndex* list1 = (EvalIndex*)pos.plist1();
                diff.p[1][0] = 0;
                diff.p[1][1] = 0;
                ...
                const EvalIndex* list0 = (EvalIndex*)pos.plist0();
                diff.p[0][0] = 0;
                diff.p[0][1] = 0;
                for (int i = 0; i < pos.nlist(); ++i) {
